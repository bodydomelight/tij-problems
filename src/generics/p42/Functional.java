...
// function objects to adapt to our particular needs:
    ...
    static class ClassBAdder implements Combiner<ClassB> {

        @Override
        public ClassB combine(ClassB x, ClassB y) {
            return new ClassB(x.getNum() + y.getNum());
        }
    }
    static class ClassAComparer implements UnaryPredicate<ClassA> {
        int length;

        public ClassAComparer(int length) {
            this.length = length;
        }
        
        @Override
        public boolean test(ClassA x) {
            return x.getDate().length() > length;
        }
...
//        print(transform(lbd, new BigDecimalUlp()));
        ...
        List<ClassA> ca = Arrays.asList(new ClassA(), new ClassA(),
                new ClassA(), new ClassA(), new ClassA());
        System.out.println(filter(ca, new ClassAComparer(8)));
        List<ClassB> cb = Arrays.asList(new ClassB(), new ClassB(),
                new ClassB(), new ClassB(), new ClassB());
        System.out.println(reduce(cb, new ClassBAdder()));
